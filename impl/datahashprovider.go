...
type DataHashProvider struct {
	...
	providerFunc lookup.DataHash
	hashes       map[string]px.OrderedMap
	hashesLock   sync.RWMutex
}

func (dh *DataHashProvider) UncheckedLookup(key lookup.Key, invocation lookup.Invocation, merge lookup.MergeStrategy) px.Value {
...
func (dh *DataHashProvider) invokeWithLocation(invocation lookup.Invocation, location lookup.Location, root string) px.Value {
	if location == nil {
...
func (dh *DataHashProvider) lookupKey(invocation lookup.Invocation, location lookup.Location, root string) px.Value {
...
func (dh *DataHashProvider) dataValue(ic lookup.Invocation, location lookup.Location, root string) px.Value {
...
func (dh *DataHashProvider) providerFunction(ic lookup.Invocation) (pf lookup.DataHash) {
	if dh.providerFunc == nil {
		...
		if f, ok := px.Load(ic, px.NewTypedName(px.NsFunction, dh.function.Name())); ok {
			dh.providerFunc = func(pc lookup.ProviderContext, options map[string]px.Value) px.OrderedMap {
...
func (dh *DataHashProvider) dataHash(ic lookup.Invocation, location lookup.Location) (hash px.OrderedMap) {
	key := ``
	opts := NoOptions
	if location != nil {
...
func newDataHashProvider(he lookup.HierarchyEntry) lookup.DataProvider {
	ls := he.Locations()
